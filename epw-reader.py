...
def write_to_excel(df, excel_file_path):
    ...
    with pd.ExcelWriter(excel_file_path, engine='xlsxwriter') as writer:
        ...
        worksheet_main.write('A1', 'Include Weekends?')
        worksheet_main.write('A2', 'No')

        # Write the default user input to specify occupied hours
        worksheet_main.write('A4', 'Occupied Hours')
        worksheet_main.write('A5', 'Start')
        worksheet_main.write('B5', 'End')
        worksheet_main.write('A6', 1)
        worksheet_main.write('B6', 24)

        # Write the default user input for including breaks
        worksheet_main.write('A8', 'Date Range to Exclude')
        worksheet_main.write('A9', 'Start')
        worksheet_main.write('B9', 'End')
        worksheet_main.write('A10', '5/15')
        worksheet_main.write('B10', '8/15')

        # Write the Main spreadsheet output
        worksheet_main.write('E1', 'Temp Ranges')
        worksheet_main.write('F1', '# of Hours')
        worksheet_main.write('G1', '% of Hours')
        worksheet_main.write_formula('E2', '="Less than "&I2')
        worksheet_main.write_formula('E3', '=I2&" to "&J2')
        worksheet_main.write_formula('E4', '=J2&" to "&K2')
        worksheet_main.write_formula('E5', '=K2&" to "&L2')
        worksheet_main.write_formula('E6', '=L2&" to "&M2')
        worksheet_main.write_formula('E7', '=M2&" to "&N2')
        worksheet_main.write_formula('E8', '=N2&" to "&O2')
        worksheet_main.write_formula('E9', '=O2&" to "&P2')
        worksheet_main.write_formula('E10', '=P2&" to "&Q2')
        worksheet_main.write_formula('E11', '="Greater than "&Q2')
        # Write total number of hours in each bin to results
        for i in range(2,12):
            worksheet_main.write_formula(f'F{i}', f'=Bins!C{i}')
        # Write % of hours in each bin to results
        for i in range(2,12):
            worksheet_main.write_formula(f'G{i}', f'=IF(sum(Raw_Data!L2:L8761) <> sum(F2:F11), "ERROR", F{i}/sum(F2:F11))')

        # Specify the values for cells I2 to Q2 in the "Main"
        bin_values = [25, 35, 45, 55, 65, 75, 85, 95, 105]
        bin_columns = ['I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q']
        worksheet_main.write_row('I2', bin_values)
        ...
        for row_num in range(2, len(df) + 2):  # Start from row 2 (header is row 1)
            ...
            formula = f'=IF(AND(D{row_num}>=Main!$A$6,D{row_num}<=Main!$B$6),1,0)'
            worksheet_raw_data.write_formula(cell_reference, formula)
